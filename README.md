...
A unified [Model Context Protocol](https://modelcontextprotocol.io) server that lets Claude query any SQLite database and build live Streamlit dashboards — all from a single conversation.
...
- [`uv`](https://docs.astral.sh/uv/) — already used in this project (see `uv.lock`)
...
Config JSON is likely invalid — trailing commas and mismatched brackets are common mistakes. Paste it into [jsonlint.com](https://jsonlint.com) to validate. Always fully quit and reopen Claude Desktop after any config change.
